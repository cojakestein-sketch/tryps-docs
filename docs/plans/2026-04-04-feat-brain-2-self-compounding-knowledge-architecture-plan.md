...
- [x] At least one `bases` dashboard working (Scope Tracker — `dashboards/scope-tracker.base`)
- [x] If `bases` insufficient, Dataview installed — N/A, bases file created, test in Obsidian
- [x] Team Org Chart canvas created (`canvases/team-org.canvas`) — full org with scope assignments
- [x] Brain Architecture canvas created (`canvases/brain-architecture.canvas`) — 3 layers, 3 operations, compounding loop
- [x] Templater templates ready — 5 templates in `templates/` (scope-state, memory-entry, standup, session-log, wiki-article). Templater plugin needs manual install in Obsidian.
- [x] At least one Marp slide deck rendering (`decks/weekly-retro.md`) — Marp plugin needs manual install in Obsidian.
- [x] At least one interactive HTML output (`dashboards/sc-velocity.html`) — Chart.js with scope bars, overall donut, bug burn-down, detail table
- [x] Jake can open Obsidian and see: org chart, scope progress, brain health — all from live data
